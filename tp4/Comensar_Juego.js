...
//Colicion bala contra los enemigos
  if (GameOver==false && Victoria == false) {
    ...
    if (VidaEnemigo3==0 && dist(395+11, 275+ round(millis()/30), posXBala, posYBala )< 11 && posYBala< (275+round(millis()/30))+16) {
      ...
      if (VidaEnemigo3==0 && dist(posXAvion+11, posYAvion, 395, 275+ round(millis()/30)) < 11 ) {
        GameOver=true;
        VidaEnemigo3=1;
      }
    }
...
//barra de puntaje
  if (GameOver==false && Victoria==false) {
    ...
    textFont(MiFuente);
    textSize(20);
    text("Segundo: "+ seReinicio, 400, 550);
    textSize(20);
    text("PUNTAJE:" + PosYFondo + round(millis()/30), 150, 550)
  }
//fin puntaje
...
//pantallas ganador y perdedor
  ...
  if (GameOver==true) {
    ...
    text("variable :" + seReinicio,300,420);
  } else {
    if (Victoria==true) {
